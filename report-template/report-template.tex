...
\addbibresource{references.bib}% import the bibliography

% TITLE | AUTHOR | DATE
\author{Name: (your user name)}% author's name
\title{Written Report – 6.419x Module 1}% name of report
\date{\today}% date of report

% Save the title, author, date
\makeatletter
\let\thetitle\@title
\let\theauthor\@author
\let\thedate\@date
...
\lhead{\theauthor}% author's name
\rhead{\thetitle}% name of report
\lfoot{\thedate}% add date
\pagestyle{fancy}% set headers
...
% command \sepline defines a gray horizontal line
\newcommand*\sepline{%
